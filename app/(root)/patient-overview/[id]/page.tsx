...
const PatientOverview = () => {
  ...
  useEffect(() => {
    const fetchPatient = async () => {
      if (!patientId) return; // Don't proceed if patientId is not available

      try {
        const fetchedPatient = await getPatientById(Number(patientId)); // Convert patientId to number if necessary
